...
public class MIPSTokenMarker extends TokenMarker {
                    ...
                    switch (c) {
                        ...
                        case ':':
                            ...
                            */
                            ...
                            try {
                                validIdentifier = com.mrlonis.mar45.mars.assembler.TokenTypes.isValidIdentifier(
    ...
    public ArrayList getTokenExactMatchHelp(Token token, String tokenText) {
        ...
        if (token != null && token.id == Token.KEYWORD1) {
            ArrayList instrMatches = com.mrlonis.mar45.mars.Globals.instructionSet.matchOperator(tokenText);
            if (!instrMatches.isEmpty()) {
                int realMatches = 0;
                matches = new ArrayList();
                for (Object instrMatch : instrMatches) {
                    Instruction inst = (Instruction) instrMatch;
                    if (com.mrlonis.mar45.mars.Globals.getSettings().getExtendedAssemblerEnabled()
    ...
    public ArrayList getTokenPrefixMatchHelp(String line, Token tokenList, Token token, String tokenText) {
        ...
        // CASE:  Current token is valid KEYWORD1 (MIPS instruction).  If this line contains a previous KEYWORD1 or
        // KEYWORD2
        //        token, then we ignore this one and do exact match on the first one.  If it does not, there may be
        // longer
        ...
        // CASE:  Current token is valid KEYWORD2 (MIPS directive).  If this line contains a previous KEYWORD1 or
        // KEYWORD2
        //        token, then we ignore this one and do exact match on the first one.  If it does not, there may be
        // longer
        ...
        if (token != null && token.id == Token.NULL) {
            ...
            if (keywordTokenText == null && !trimmedTokenText.isEmpty()) {
                ...
                } else if (trimmedTokenText.length()
                        >= com.mrlonis.mar45.mars.Globals.getSettings().getEditorPopupPrefixLength()) {
    ...
    private ArrayList getTextFromInstructionMatch(String tokenText, boolean exact) {
        ...
        if (exact) {
            matches = com.mrlonis.mar45.mars.Globals.instructionSet.matchOperator(tokenText);
        } else {
            matches = com.mrlonis.mar45.mars.Globals.instructionSet.prefixMatchOperator(tokenText);
        }
        if (matches == null) {
            return null;
        }
    ...
    /**
     * Get KeywordMap containing all MIPS key words. This includes all instruction mnemonics, assembler directives, and
     * register names.
     *
    ...
    public static KeywordMap getKeywords() {
        if (cKeywords == null) {
            ...
            java.util.ArrayList instructionSet = com.mrlonis.mar45.mars.Globals.instructionSet.getInstructionList();
            for (Object object : instructionSet) {
                cKeywords.add(((Instruction) object).getName(), Token.KEYWORD1);
            }
            // add assembler directives
            java.util.ArrayList directiveSet = com.mrlonis.mar45.mars.assembler.Directives.getDirectiveList();
            for (Object o : directiveSet) {
                cKeywords.add(((Directives) o).getName(), Token.KEYWORD2);
            }
            // add integer register file
            com.mrlonis.mar45.mars.mips.hardware.Register[] registerFile =
                    com.mrlonis.mar45.mars.mips.hardware.RegisterFile.getRegisters();
            ...
            com.mrlonis.mar45.mars.mips.hardware.Register[] coprocessor1RegisterFile =
                    com.mrlonis.mar45.mars.mips.hardware.Coprocessor1.getRegisters();
            for (com.mrlonis.mar45.mars.mips.hardware.Register register : coprocessor1RegisterFile) {
    ...
    private boolean tokenListContainsKeyword() {
        ...
        while (token != null) {
            ...
            if (token.id == Token.KEYWORD1 || token.id == Token.KEYWORD2 || token.id == Token.KEYWORD3) result = true;
            token = token.next;
        }
