...
import com.mrlonis.mar45.mars.ProgramStatement;
import com.mrlonis.mar45.mars.mips.hardware.AccessNotice;
import com.mrlonis.mar45.mars.mips.hardware.AddressErrorException;
import com.mrlonis.mar45.mars.mips.hardware.Memory;
import com.mrlonis.mar45.mars.mips.hardware.MemoryAccessNotice;
import com.mrlonis.mar45.mars.mips.instructions.BasicInstruction;
import com.mrlonis.mar45.mars.mips.instructions.BasicInstructionFormat;
import com.mrlonis.mar45.mars.venus.RunAssembleAction;
import com.mrlonis.mar45.mars.venus.RunBackstepAction;
import com.mrlonis.mar45.mars.venus.RunStepAction;
import com.mrlonis.mar45.mars.venus.VenusUI;
...
public class MipsXray extends AbstractMarsToolAndApplication {
    ...
    protected JComponent buildMainDisplayArea() {
        ...
        try {
            BufferedImage im = ImageIO.read(getClass().getResource(Globals.imagesPath + "datapath.png"));
            ...
            System.out.println(
                    "Load Image error for " + getClass().getResource(Globals.imagesPath + "datapath.png") + ":\n" + e);
            e.printStackTrace();
        }
    ...
    protected JComponent buildMainDisplayArea(String figure) {
        ...
        gc = ge.getDefaultScreenDevice().getDefaultConfiguration();
        try {
            BufferedImage im = ImageIO.read(getClass().getResource(Globals.imagesPath + figure));
            ...
            System.out.println(
                    "Load Image error for " + getClass().getResource(Globals.imagesPath + figure) + ":\n" + e);
            e.printStackTrace();
        }
        System.setProperty("sun.java2d.translaccel", "true");
    ...
    class Vertex {
        ...
        public Vertex(
            ...
            this.color = new Color(
                    Integer.parseInt(listColor[0]), Integer.parseInt(listColor[1]), Integer.parseInt(listColor[2]));
        }
    ...
    // Internal class that set the parameters value, control the basic behavior of the animation , and execute the
    // animation of the
    ...
    class DatapathAnimation extends JPanel implements ActionListener, MouseListener {
        ...
        public void loadHashMapValues() {
            ...
            importXmlStringData(
                    "/MipsXRayOpcode.xml", functionEquivalenceTable, "function_equivalence", "bits", "mnemonic");
            importXmlStringData(
                    "/MipsXRayOpcode.xml", registerEquivalenceTable, "register_equivalence", "bits", "mnemonic");
            importXmlDatapathMap("/MipsXRayOpcode.xml", "datapath_map");
        }

        // import the list of opcodes of mips set of instructions
        public void importXmlStringData(
                String xmlName, HashMap table, String elementTree, String tagId, String tagData) {
            ...
            try {
                ...
                for (int i = 0; i < equivalenceList.getLength(); i++) {
                    ...
                    for (int j = 0; j < bitsList.getLength(); j++) {
                        table.put(
                                bitsList.item(j).getTextContent(),
                                mnemonic.item(j).getTextContent());
                    }
                }
            } catch (Exception e) {
                e.printStackTrace();
        ...
        public void importXmlDatapathMap(String xmlName, String elementTree) {
            ...
            try {
                ...
                for (int i = 0;
                        i < datapath_mapList.getLength();
                        i++) { // extract the vertex of the xml input and encapsulate into the vertex object
                    ...
                    if ("000000".equals(instructionCode.substring(0, 6))) { // R-type instructions
                    ...
                    } else if (instructionCode
                            .substring(0, 6)
                            .matches("100[0-1][0-1][0-1]")) { // LOAD type instructions
                        color = datapath_mapItem.getElementsByTagName("color_LOADtype");
                        // System.out.println("load type");
                    } else if (instructionCode
                            .substring(0, 6)
                            .matches("101[0-1][0-1][0-1]")) { // LOAD type instructions
                    ...
                    for (int j = 0; j < index_vertex.getLength(); j++) {
                        Vertex vert = new Vertex(
                                Integer.parseInt(index_vertex.item(j).getTextContent()),
                                Integer.parseInt(init.item(j).getTextContent()),
                                Integer.parseInt(end.item(j).getTextContent()),
                                name.item(j).getTextContent(),
                                Integer.parseInt(other_axis.item(j).getTextContent()),
                                Boolean.parseBoolean(isMovingXaxis.item(j).getTextContent()),
                                color.item(j).getTextContent(),
                                targetVertex.item(j).getTextContent(),
        ...
        public void setUpInstructionInfo(Graphics2D g2d) {
            ...
            if ("000000"
                    .equals(instructionCode.substring(0, 6))) { // R-type instructions description on screen definition.
                ...
                // instruction name
                textVariable =
                        new TextLayout(functionEquivalenceTable.get(instructionCode.substring(26, 32)), font, frc);
                g2d.setColor(Color.BLACK);
                textVariable.draw(g2d, 25, 500);

                // register in RS
                textVariable =
                        new TextLayout(registerEquivalenceTable.get(instructionCode.substring(6, 11)), font, frc);
                g2d.setColor(Color.BLACK);
                textVariable.draw(g2d, 65, 500);

                // register in RT
                textVariable =
                        new TextLayout(registerEquivalenceTable.get(instructionCode.substring(16, 21)), font, frc);
                g2d.setColor(Color.BLACK);
                textVariable.draw(g2d, 105, 500);

                // register in RD
                textVariable =
                        new TextLayout(registerEquivalenceTable.get(instructionCode.substring(11, 16)), font, frc);
                g2d.setColor(Color.BLACK);
                ...
                textVariable = new TextLayout(
                        ...
                        frc); // description of instruction code type for jump.
                g2d.setColor(Color.black);
                textVariable.draw(g2d, 280, 30);
                ...
                textVariable = new TextLayout(
                        ...
                        new Font("Verdana", Font.BOLD, 25),
                        frc); // description of instruction code type for load.
                g2d.setColor(Color.black);
                textVariable.draw(g2d, 280, 30);
                // opcode label
                textVariable = new TextLayout("opcode", fontTitle, frc);
                ...
                textVariable = new TextLayout(
                        "M[ " + registerEquivalenceTable.get(instructionCode.substring(16, 21)) + " + "
                                + parseBinToInt(instructionCode.substring(6, 32)) + " ]",
                        font,
                        frc);
                g2d.setColor(Color.BLACK);
                textVariable.draw(g2d, 105, 500);
        ...
        // set the initial state of the variables that controls the animation, and start the timer that triggers the
        // animation.
        ...
        public void printTrackLtoR(Vertex v) {
            ...
            track = new int[size];
            for (int i = 0; i < size; i++) track[i] = v.getInit() + i;
            if (v.isActive()) {
        ...
        // public boolean printTrackRtoL(int init, int end ,int currentIndex, Graphics2D g2d, Color color, int
        // otherAxis,
        ...
        // public boolean printTrackDtoU(int init, int end ,int currentIndex, Graphics2D g2d, Color color, int
        // otherAxis,
        //		 boolean active, boolean firstInteraction){
        ...
        // public boolean printTrackUtoD(int init, int end ,int currentIndex, Graphics2D g2d, Color color, int
        // otherAxis,
        //		 boolean active,  boolean firstInteraction){
        ...
        public void printTextDtoU(Vertex v) {
            ...
            if (instructionCode.substring(0, 6).matches("0001[0-1][0-1]") && "CP+4".equals(v.getName())) // branch code
            actionInFunctionalBlock = new TextLayout("PC+OFFSET", new Font("Verdana", Font.BOLD, 13), frc);

            if ("WRITING".equals(v.getName())) {
        ...
        public String parseBinToInt(String code) {
            ...
            for (int i = code.length() - 1; i >= 0; i--) {
                if ("1".equals(code.substring(i, i + 1))) {
                    value += (int) Math.pow(2, code.length() - i - 1);
                }
            }
        ...
        // verifies the previous status of the animation and increment the position of each line that interconnect the
        // unit function.
                        ...
                        if (!vert.isActive()) {
                            ...
                            for (int k = 0; k < j; k++) {
                                ...
                                for (Vertex vertex : vertexTraversed) {
                                    if (tempVertex.getNumIndex() == vertex.getNumIndex()) hasThisVertex = true;
                                }
                                if (!hasThisVertex) {
                                    outputGraph.get(vert.getNumIndex()).get(k).setActive(true);
                                    vertexTraversed.add(
                                            outputGraph.get(vert.getNumIndex()).get(k));
                                }
                            }
                        }
                    } else {
