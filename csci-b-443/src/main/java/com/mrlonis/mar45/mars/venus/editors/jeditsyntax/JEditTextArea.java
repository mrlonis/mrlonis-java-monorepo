...
import com.mrlonis.mar45.mars.Settings;
...
/**
 * jEdit's text area component. It is more suited for editing program source code than JEditorPane, because it drops the
 * unnecessary features (images, variable-width lines, and so on) and adds a whole bunch of useful goodies such as:
 *
 * <ul>
 ...
 * </ul>
 *
 * It is also faster and doesn't have as many problems. It can be used in other applications; the only other part of
 * jEdit it depends on is the syntax package.
 *
 * <p>To use it in your app, treat it like any other component, for example:
 *
...
public class JEditTextArea extends JComponent {
    ...
    public JEditTextArea(TextAreaDefaults defaults, JComponent lineNumbers) {
        ...
        JScrollPane lineNumberScroller = new JScrollPane(
                lineNumbers,
    ...
    public int getLineEndOffset(int line) {
        Element lineElement = document.getDefaultRootElement().getElement(line);
        if (lineElement == null) return -1;
        else return lineElement.getEndOffset();
    }
    ...
    class ScrollLayout implements LayoutManager {
        public void addLayoutComponent(String name, Component comp) {
            if (CENTER.equals(name)) center = comp;
            else if (RIGHT.equals(name)) right = comp;
            else if (BOTTOM.equals(name)) bottom = comp;
            else if (LEFT_OF_SCROLLBAR.equals(name)) leftOfScrollBar.addElement(comp);
        }

        public void removeLayoutComponent(Component comp) {
            if (center == comp) center = null;
            if (right == comp) right = null;
            if (bottom == comp) bottom = null;
    ...
    /**
     * Constructs string for auto-indent feature. Returns empty string if auto-intent is disabled or if line has no
     * leading white space. Uses getLeadingWhiteSpace(). Is used by InputHandler when processing key press for Enter
     * key. DPS 31-Dec-2010
     *
     * @return String containing auto-indent characters to be inserted into text
     */
    public String getAutoIndent() {
        if (Globals.getSettings().getBooleanSetting(Settings.AUTO_INDENT)) {
    ...
    /**
     * Makes a copy of leading white space (tab or space) from the current line and returns it. DPS 31-Dec-2010
     *
        ...
        if (tokenMarker != null) {
            ...
            if (tokenAtOffset != null) {
                ...
                if (exact) {
                    ...
                    matches = tokenMarker.getTokenPrefixMatchHelp(
                            lineSegment.toString(), tokenList, tokenAtOffset, tokenText);
                }
            }
        }
        return matches;
    ...
    private void applySyntaxSensitiveHelp() {
        if (!com.mrlonis.mar45.mars.Globals.getSettings()
                .getBooleanSetting(com.mrlonis.mar45.mars.Settings.POPUP_INSTRUCTION_GUIDANCE)) {
            return;
        }
        int line = getCaretLine();
        int lineStart = getLineStartOffset(line);
    ...
    private void checkPopupMenu(KeyEvent evt) {
        ...
        if ((evt.getKeyCode() == KeyEvent.VK_ENTER || evt.getKeyCode() == KeyEvent.VK_ESCAPE)
                && popupMenu != null
                && popupMenu.isVisible()) popupMenu.setVisible(false);
    }
        ...
        if ((evt.getKeyCode() == KeyEvent.VK_TAB || evt.getKeyCode() == KeyEvent.VK_ENTER)
            ...
            if (item.isEnabled()) {
                ...
                if (listeners.length > 0) {
                    listeners[0].actionPerformed(new ActionEvent(
                            item, ActionEvent.ACTION_FIRST, (evt.getKeyCode() == KeyEvent.VK_TAB) ? "\t" : " "));
                    return true;
                }
            }
        }
