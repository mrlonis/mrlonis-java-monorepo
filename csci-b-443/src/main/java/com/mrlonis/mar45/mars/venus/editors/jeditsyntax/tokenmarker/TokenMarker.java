...
package com.mrlonis.mar45.mars.venus.editors.jeditsyntax.tokenmarker;

import java.util.*;
import javax.swing.text.Segment;

/**
 * A token marker that splits lines of text into tokens. Each token carries a length field and an indentification tag
 * that can be mapped to a color for painting that token.
 *
 * <p>For performance reasons, the linked list of tokens is reused after each line is tokenized. Therefore, the return
 * value of <code>markTokens</code> should only be used for immediate painting. Notably, it cannot be cached.
 *
 * @author Slava Pestov
...
public abstract class TokenMarker {
    ...
    /**
     * An abstract method that splits a line up into tokens. It should parse the line, and call <code>addToken()</code>
     * to add syntax tokens to the token list. Then, it should return the initial token type for the next line.
     *
     * <p>For example if the current line contains the start of a multiline comment that doesn't end on that line, this
    ...
    /**
     * Returns if the token marker supports tokens that span multiple lines. If this is true, the object using this
     * token marker is required to pass all lines in the document to the <code>markTokens()</code> method (in turn).
     *
     * <p>The default implementation returns true; it should be overridden to return false on simpler token markers for
     * increased speed.
     */
    ...
    /**
     * Informs the token marker that lines have been inserted into the document. This inserts a gap in the <code>
     * lineInfo</code> array.
     *
    ...
    /**
     * Returns true if the next line should be repainted. This will return true after a line has been tokenized that
     * starts a multiline token that continues onto the next line.
     */
    ...
    /**
     * Construct and return any appropriate help information for the given token. This default definition returns null;
    ...
    /**
     * Construct and return any appropriate help information for the given token or "token prefix". Will match
     * instruction prefixes, e.g. "s" matches "sw". This default definition returns null; override it in
    ...
    /**
     * The last token in the list. New tokens are added here. This should be set to null before a new line is to be
     * tokenized.
     */
    protected Token lastToken;

    /**
     * An array for storing information about lines. It is enlarged and shrunk automatically by the <code>insertLines()
     * </code> and <code>deleteLines()</code> methods.
     */
    protected LineInfo[] lineInfo;

    /**
     * The number of lines in the model being tokenized. This can be less than the length of the <code>lineInfo</code>
     * array.
     */
    ...
    /**
     * Creates a new <code>TokenMarker</code>. This DOES NOT create a lineInfo array; an initial call to <code>
     * insertLines()</code> does that.
     */
    ...
    /**
     * Ensures that the <code>lineInfo</code> array can contain the specified index. This enlarges it if necessary. No
     * action is taken if the array is large enough already.
     *
     * <p>It should be unnecessary to call this under normal circumstances; <code>insertLine()</code> should take care
     * of enlarging the line info array automatically.
     *
    ...
    public class LineInfo {
        ...
        /**
         * This is for use by the token marker implementations themselves. It can be used to store anything that is an
         * object and that needs to exist on a per-line basis.
         */
        public Object obj;
    }
}
