...
public class HardcopyWriter extends Writer {
     ...
     * The jobname appears left justified at the top of each printed page. The font size is specified in points, as
     * on-screen font sizes are. The margins are specified in inches (or fractions of inches).
     */
    public HardcopyWriter(
        ...
        /**
         * ***************************************************** SANDERSON OVERRIDE 8-17-2004: I didn't like the results
         * produced by the code below, so am commenting it out and just setting pagedpi to 72. This assures, among other
         * things, that the client asking for 10 point font will really get 10 point font!
         *
         * <p>pagesize = job.getPageDimension( ); // query the page size pagedpi = job.getPageResolution( ); // query
         * the page resolution // Bug Workaround: // On Windows, getPageDimension( ) and getPageResolution don't work,
         * so // we've got to fake them. if (System.getProperty("os.name").regionMatches(true,0,"windows",0,7)){ // Use
         * screen dpi, which is what the PrintJob tries to emulate pagedpi = toolkit.getScreenResolution( ); // Assume a
         * 8.5" x 11" page size. A4 paper users must change this. pagesize = new Dimension((int)(8.5 * pagedpi),
         * 11*pagedpi); // We also have to adjust the fontsize. It is specified in points, // (1 point = 1/72 of an
         * inch) but Windows measures it in pixels. fontsize = fontsize * pagedpi / 72; }
         * *********************************
         */
        pagedpi = 72;
    ...
    /**
     * This is the write( ) method of the stream. All Writer subclasses implement this. All other versions of write( )
    ...
    /**
     * This is the flush( ) method that all Writer subclasses must implement. There is no way to flush a PrintJob
     * without prematurely printing the page, so we don't do anything.
     */
    public void flush() {
        /* do nothing */
    }
    /**
     * This is the close( ) method that all Writer subclasses must implement. Print the pending page (if any) and
     * terminate the PrintJob.
     */
    public void close() {
    ...
    /**
     * Set the font style. The argument should be one of the font style constants defined by the java.awt.Font class.
     * All subsequent output will be in that style. This method relies on all styles of the Monospaced font having the
     * same metrics.
     */
    ...
    /**
     * This is the exception class that the HardcopyWriter constructor throws when the user clicks "Cancel" in the print
     * dialog box.
     */
