...
/**
 * Demo of Mars tool capability. Ken Vollmar, 27 Oct 2006 KenVollmar@missouristate.edu This tool displays movements by a
 * series of players in a game of ScavengerHunt. Players will read and write MIPS memory-mapped locations to move and
 * regain energy. See accompanying documentation for memory-mapped addresses, rules of the game, etc.
 */
public class ScavengerHunt implements Observer, MarsTool {
    ...
    private static final int ADDR_GAME_ON =
            0xffffe008; // MIPS byte address of signal that administration has initialized data
    ...
    private class PlayerData {
        ...
        public long getFinishMin() {
            return (finishTime / 60_000);
        } // Minutes portion of finishTime

        public long getFinishSec() {
            return (finishTime % 60_000) / 1000; // Seconds portion of finishTime
    ...
    private class ScavengerHuntRunnable implements Runnable {
        ...
        public ScavengerHuntRunnable() // constructor
            ...
            frame.setDefaultCloseOperation(
                    JFrame.EXIT_ON_CLOSE); // TBD --- This should close only the Tool, not the entire MARS
    ...
    /* ------------------------------------------------------------------------- */
    /**
     * ScavengerHuntDisplay does not have access to the same MIPS Memory class object used by the ScavengerHunt class
     * object. Need read-only access to a similar data structure maintained within ScavengerHunt.
     */
    private class ScavengerHuntDisplay extends JPanel {
        ...
        /**
         * paintComponent does not have access to the same MIPS Memory class object used by the ScavengerHunt class
         * object. Need read-only access to a similar data structure maintained within ScavengerHunt.
         */
        public void paintComponent(Graphics g) {
            ...
            g2.drawLine(
                    width - 120, 35, width - 120, 35 + (NUM_PLAYERS * 15)); // vertical line for location-visited marks
            g2.drawLine(
                    width - 50, 35, width - 50, 35 + (NUM_PLAYERS * 15)); // vertical line for location-visited marks
            for (int i = 0; i < NUM_PLAYERS; i++) {
                ...
                if (pd[i].isFinished()) {
                    ...
                    // This doesn't display leading zeroes to align time components (e.g. 27 millisec ought to print as
                    // 027)
                    g2.drawString(
            ...
            // System.out.println("paintComponent: Player " + 0 + " is at (" + pd[0].getWhereAmIX() + ", " +
            // pd[0].getWhereAmIY() + ")" );
    ...
    private int toolReadPlayerData(int p, int offset) {
        ...
        if (KENVDEBUG) {
            System.out.println("ScavengerHunt.toolReadPlayerData: called with player " + p + ", offset = "
                    + Binary.intToHexString(offset)
    ...
    private void toolWritePlayerData(int p, int offset, int data) {
        ...
        if (KENVDEBUG) {
            System.out.println("ScavengerHunt.toolWritePlayerData: called with player " + p + ", offset = "
                    + Binary.intToHexString(offset) + ", data = "
                    + Binary.intToHexString(data));
        }
    ...
    private void initializeScavengerData() {
        ...
        { // Two coordinates (x and y) for each location
            ...
            loc[j].Y = (int)
                    (randomStream.nextDouble() * (GRAPHIC_HEIGHT - 50)); // Y coord. (leave room for buttons in window)
        ...
        for (int i = 0; i < NUM_PLAYERS; i++) // Initialize data for each player
        {
            ...
            {
                toolWritePlayerData(
                        i, OFFSET_LOC_ARRAY + (j * 8) + 0, loc[j].X); // Set the same locations for each player
                toolWritePlayerData(
                        i, OFFSET_LOC_ARRAY + (j * 8) + 4, loc[j].Y); // Set the same locations for each player
            }
